...
public class PerfilController implements Initializable {
    ...
    private void cargarBibliotecaUsuario(int idUsuario) {
        ...
        if (registrosAdquisiciones != null && !registrosAdquisiciones.isEmpty()) {
            for (Biblioteca registro : registrosAdquisiciones) {
                //Buscar los datos de la aplicación uno a uno por su ID correlativa
    ...
    @FXML
    public void cerrarSesionActiva(ActionEvent event) {
        try {
            //Limpiamos el usuario de la sesion global
            Sesion.setUsuarioActual(null);
            //cierra todas las ventanas
            List<Window> ventanas = new ArrayList<>(Stage.getWindows());
            ...
            System.err.println("Error al cerrar ventanas y abrir el Login: " + e.getMessage());
            e.printStackTrace();
        }
    }
}